...
// Formato de números enteros (semanas, conteos)
...
export const ComparacionMeses = () => {
  ...
  const generarDatosMes = (ano: string, mes: string) => {
    const regs = registros.filter(
      r => r.ano.toString() === ano && r.mes === mes && (taller === 'Todos' || r.taller === taller)
    );
    // "cantidad" = ventas logradas en el mes (monto en dólares)
    const cantidad = regs.reduce((acc, r) => acc + (r.logrado || 0), 0);
    const meta = regs.reduce((acc, r) => acc + (r.meta || 0), 0);
    const faltante = Math.max(meta - cantidad, 0);
    // Semanas capturadas del mes: cada detalle de venta es una semana
    const semanas = Math.max(1, regs.reduce((acc, r) => acc + (r.detalles ? r.detalles.length : 0), 0));
    return {
      ano, mes,
      etiqueta: `${mes} ${ano}`,
      cantidad, meta, faltante, semanas,
  ...
  const periodo1 = useMemo(
    ...
    [registros, taller, ano1, mes1]
  );
  const periodo2 = useMemo(
    () => generarDatosMes(ano2, mes2),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [registros, taller, ano2, mes2]
  );
  ...
  const datasetLeft = useMemo(() => {
    ...
    return [
      { id: 'L1', label: `${base.mes} — Ventas`, ventas: base.cantidad, color: '#1d8cf8' },
      { id: 'L2', label: `${base.mes} — Meta`, ventas: base.meta, color: '#00d6b4' },
      { id: 'L3', label: `${reciente.mes} — Ventas`, ventas: reciente.cantidad, color: '#ffbc11' },
          ...
          <div style={{ position: 'absolute', top: '50%', left: '50%', width: 0, height: 0, zIndex: 100 }}>
            {pieData.map(op => {
              ...
              return (
                ...
                >
                  <span style={{ fontSize: '0.78rem', fontWeight: 800, color: 'var(--text-main)', whiteSpace: 'nowrap' }}>{miFormatearMoneda(op.ventas)}</span>
            ...
            return (
              ...
              >
                <div style={{ position: 'absolute', top: '-35px', left: '50%', transform: 'translateX(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <span style={{ fontSize: '0.8rem', fontWeight: 800, color: 'var(--text-main)', whiteSpace: 'nowrap', textShadow: is3D ? '0 2px 4px rgba(0,0,0,0.8)' : 'none' }}>{miFormatearMoneda(op.ventas)}</span>
            ...
            return (
              ...
              >
                <div style={{ position: 'absolute', top: '-35px', left: '50%', transform: 'translateX(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', opacity: isHovered ? 1 : (isDimmed ? 0 : 1), transition: 'opacity 0.2s' }}>
                  <span style={{ fontSize: '0.75rem', fontWeight: 800, color: 'var(--text-main)', whiteSpace: 'nowrap' }}>{miFormatearMoneda(op.ventas)}</span>
                  <span style={{ fontSize: '0.65rem', fontWeight: 800, color: op.color, whiteSpace: 'nowrap' }}>{op.pctStr}%</span>
                </div>
              </div>
            );
            ...
            return (
              ...
              >
                <div style={{ position: 'absolute', top: '-35px', left: '50%', transform: 'translateX(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', opacity: isHovered ? 1 : (isDimmed ? 0 : 1), transition: 'opacity 0.2s', pointerEvents: 'none' }}>
                  <span style={{ fontSize: '0.75rem', fontWeight: 800, color: 'var(--text-main)', whiteSpace: 'nowrap', textShadow: is3D ? '0 2px 4px rgba(0,0,0,0.8)' : 'none' }}>{miFormatearMoneda(op.ventas)}</span>
                  <span style={{ fontSize: '0.65rem', fontWeight: 800, color: op.color, whiteSpace: 'nowrap' }}>{op.pctStr}%</span>
                </div>
              </div>
            );
    ...
    <div className="animate-in fade-in">
      <div className="page-header">
        <div className="page-title">
          ...
          <div>
            ...
            <p className="page-subtitle">Ventas mes contra mes, incluso de años distintos</p>
          </div>
        </div>
      </div>
      ...
      {mismoMes ? (
        ...
        <div className="card" style={{ textAlign: 'center', padding: '4rem 2rem' }}>
          ...
          <h3 style={{ color: 'var(--text-main)', marginBottom: '0.5rem' }}>Sin registros de ventas</h3>
          ...
          {semanasDistintas && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', backgroundColor: 'var(--bg-highlight)', border: '1px solid var(--border)', borderLeft: '4px solid #7c3aed', borderRadius: '8px', padding: '0.8rem 1.1rem', marginBottom: '1.5rem' }}>
              ...
              <span style={{ fontSize: '0.82rem', color: 'var(--text-main)', fontWeight: 600 }}>
                Estos meses tienen distinta cantidad de semanas capturadas ({base.etiqueta}: {base.semanas} · {reciente.etiqueta}: {reciente.semanas}).
          ...
          <div className="comparison-grid comparison-grid-2x2">
            ...
            <div className="card" style={{ padding: 0, overflow: 'hidden', order: 1 }}>
              ...
              <div style={{ overflowX: 'auto' }}>
                <table className="table" style={{ width: '100%' }}>
                  ...
                  <tbody>
                    <tr>
                      <td><strong style={{ color: 'var(--text-main)' }}>Ventas logradas</strong></td>
                      <td style={{ textAlign: 'right', fontWeight: 700, whiteSpace: 'nowrap' }}>{base.tieneDatos ? miFormatearMoneda(base.cantidad) : '-'}</td>
                      <td style={{ textAlign: 'right', fontWeight: 700, color: 'var(--primary)', whiteSpace: 'nowrap' }}>{reciente.tieneDatos ? miFormatearMoneda(reciente.cantidad) : '-'}</td>
                      <td style={{ textAlign: 'right', backgroundColor: 'var(--bg-highlight)' }}>{celdaVariacion(variacion(base.cantidad, reciente.cantidad), true)}</td>
                    </tr>
                    <tr>
                      <td><strong style={{ color: 'var(--text-main)' }}>Meta del mes</strong></td>
                      <td style={{ textAlign: 'right', fontWeight: 700, whiteSpace: 'nowrap' }}>{base.meta > 0 ? miFormatearMoneda(base.meta) : '-'}</td>
                      <td style={{ textAlign: 'right', fontWeight: 700, color: 'var(--primary)', whiteSpace: 'nowrap' }}>{reciente.meta > 0 ? miFormatearMoneda(reciente.meta) : '-'}</td>
                      <td style={{ textAlign: 'right', backgroundColor: 'var(--bg-highlight)' }}>{celdaVariacion(variacion(base.meta, reciente.meta), true)}</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                      <td><strong style={{ color: 'var(--text-main)' }}>Semanas capturadas</strong></td>
                      <td style={{ textAlign: 'right', fontWeight: 700, color: 'var(--text-main)' }}>{base.tieneDatos ? `${base.semanas} sem` : '-'}</td>
                      <td style={{ textAlign: 'right', fontWeight: 700, color: 'var(--primary)' }}>{reciente.tieneDatos ? `${reciente.semanas} sem` : '-'}</td>
                    ...
                    <tr>
                      <td>
                        ...
                        <small style={{ display: 'block', color: 'var(--text-muted)', fontSize: '0.7rem' }}>Ventas ÷ semanas</small>
                      </td>
                      <td style={{ textAlign: 'right', fontWeight: 700, whiteSpace: 'nowrap' }}>{base.tieneDatos ? miFormatearMoneda(base.promedioSemanal) : '-'}</td>
                      <td style={{ textAlign: 'right', fontWeight: 700, color: 'var(--primary)', whiteSpace: 'nowrap' }}>{reciente.tieneDatos ? miFormatearMoneda(reciente.promedioSemanal) : '-'}</td>
                      <td style={{ textAlign: 'right', backgroundColor: 'var(--bg-highlight)' }}>{celdaVariacion(variacion(base.promedioSemanal, reciente.promedioSemanal), true)}</td>
                    </tr>
                  </tbody>
                  <tfoot>
                    <tr style={{ backgroundColor: 'var(--bg-highlight)', borderTop: '2px solid var(--border)' }}>
                      <td style={{ padding: '1rem' }}><strong style={{ fontSize: '1rem' }}>FALTANTE PARA LA META</strong></td>
                      <td style={{ textAlign: 'right', padding: '1rem', fontWeight: 800, color: base.faltante === 0 ? 'var(--success)' : 'var(--danger)', whiteSpace: 'nowrap' }}>{!base.tieneDatos ? '-' : base.faltante === 0 ? 'Meta alcanzada \u2713' : miFormatearMoneda(base.faltante)}</td>
                      <td style={{ textAlign: 'right', padding: '1rem', fontWeight: 800, color: reciente.faltante === 0 ? 'var(--success)' : 'var(--danger)', whiteSpace: 'nowrap' }}>{!reciente.tieneDatos ? '-' : reciente.faltante === 0 ? 'Meta alcanzada \u2713' : miFormatearMoneda(reciente.faltante)}</td>
                      <td style={{ textAlign: 'right', padding: '1rem' }}>{celdaVariacion(variacion(base.faltante, reciente.faltante), true)}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
              ...
              <table className="table" style={{ width: '100%' }}>
                  ...
                  {datasetRight.map(d => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td style={{ textAlign: 'right', fontWeight: 600, whiteSpace: 'nowrap' }}>{d.ventas > 0 ? miFormatearMoneda(d.ventas) : '-'}</td>
                ...
                <tfoot>
                  <tr style={{ backgroundColor: 'var(--bg-highlight)', borderTop: '2px solid var(--border)' }}>
                    ...
                    <td style={{ textAlign: 'right', padding: '1rem', fontWeight: 700, color: 'var(--text-main)', whiteSpace: 'nowrap' }}>{miFormatearMoneda(totalAmbosPeriodos)}</td>
                  ...
                  <tr style={{ backgroundColor: crecimiento >= 0 ? 'rgba(0, 214, 180, 0.08)' : 'rgba(255, 141, 114, 0.08)', borderTop: `2px solid ${crecimiento >= 0 ? 'var(--success)' : 'var(--danger)'}` }}>
                    ...
                    <td style={{ textAlign: 'right', padding: '1rem', fontWeight: 800, color: crecimiento >= 0 ? 'var(--success)' : 'var(--danger)', whiteSpace: 'nowrap' }}>
                      {difAbsoluta >= 0 ? '+' : '-'}{miFormatearMoneda(Math.abs(difAbsoluta))}
                    </td>
            ...
            <div className="card" style={{ order: 3 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', borderBottom: '1px solid var(--border)', paddingBottom: '1rem' }}>
                <h3 className="detail-section-title" style={{ border: 'none', margin: 0 }}>Ventas vs Meta</h3>
                ...
                {datasetLeft.map(d => (
                  <li key={`leg-${d.id}`} onMouseEnter={() => setHoveredId1(d.id)} onMouseLeave={() => setHoveredId1(null)}
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', flex: 1, alignItems: 'center', gap: '0.75rem' }}>
                      <span>{d.label}</span>
                      <strong style={{ color: 'var(--text-main)', fontWeight: 600, whiteSpace: 'nowrap' }}>{miFormatearMoneda(d.ventas)}</strong>
                    </div>
                  </li>
                ))}
              </ul>
