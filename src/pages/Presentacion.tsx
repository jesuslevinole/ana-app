...
import {
  ...
  Folder, Filter, ChevronDown, Store, CalendarRange, Layers
} from 'lucide-react';
...
// Parte un título en renglones por sus separadores (":" y "-"), para que cada
// idea quede en su propia línea centrada. Sin separadores devuelve una sola.
const partirTitulo = (titulo: string): string[] => {
  const partes = titulo
    .split(/\s*[:\u2013\u2014-]\s+|\s+[:\u2013\u2014-]\s*/)
    .map(x => x.trim())
    .filter(Boolean);
  return partes.length > 0 ? partes : [titulo];
};

// Título con el que llega la portada de toda presentación nueva. Es editable:
// solo es el punto de partida que pidió la gerencia.
const TITULO_PORTADA_DEFECTO = "KPI's : Key Performance Indicators - Indicadores clave de desempeño";
...
export const Presentacion = () => {
  ...
  // 'nueva' genera el mes completo, 'una' edita un taller y 'masiva' aplica
  // los mismos cambios a todas las presentaciones de la carpeta abierta.
  const [modoEditor, setModoEditor] = useState<'nueva' | 'una' | 'masiva'>('nueva');
  ...
  const abrirNueva = () => {
    ...
    setPortada({ ...DIAPOSITIVA_VACIA, activa: true, titulo: TITULO_PORTADA_DEFECTO });
  ...
  // Edición masiva: se parte de la primera presentación de la carpeta y lo que
  // se guarde se aplica a todas ellas, respetando el taller de cada una.
  const abrirEditarTodas = () => {
    const base = listaCarpeta[0];
    if (!base) return;
    setModoEditor('masiva');
    setEditandoId(null);
    setDescripcion(base.descripcion || '');
    setVistas([...base.vistas]);
    setFiltroAno(base.ano || '');
    setFiltroTaller('');
    setFiltroMes(base.mes || '');
    setFiltroSemanas(base.semanas || '');
    setPortada(normalizarTexto(base.portada));
    setCierre(normalizarTexto(base.cierre));
    setModalAbierto(true);
  };
  ...
  const guardar = async () => {
    ...
    // Sin módulos la presentación no muestra ningún dato: no se permite guardar
    if (vistas.length === 0) {
      alert('Agrega al menos un módulo para mostrar.\n\nMárcalo en "Módulos a mostrar" y aparecerá en el orden de las diapositivas.');
      return;
    }
    ...
    try {
      ...
      // --- EDICIÓN MASIVA: los mismos cambios en toda la carpeta ---
      if (modoEditor === 'masiva') {
        const objetivo = listaCarpeta;
        if (objetivo.length === 0) { setModalAbierto(false); return; }
        await Promise.all(objetivo.map(pres => guardarPresentacion({
          id: pres.id,
          nombre: nombre.trim(),
          descripcion: descripcion.trim(),
          vistas,
          ano: filtroAno,
          taller: pres.taller,      // cada una conserva su propio taller
          mes: filtroMes,
          semanas: filtroSemanas,
          portada,
          cierre,
          creadoEn: pres.creadoEn,
          orden: pres.orden,
        })));
        setModalAbierto(false);
        return;
  ...
  // Borra de una vez todas las presentaciones de la carpeta abierta
  const eliminarCarpeta = async () => {
    const objetivo = listaCarpeta;
    if (objetivo.length === 0) return;

    const aviso =
      `¿Eliminar las ${objetivo.length} presentaciones de ${rutaCarpeta}?\n\n` +
      objetivo.map(x => `· ${x.taller || 'Sin taller'}`).join('\n') +
      '\n\nEsta acción no se puede deshacer.';
    if (!confirm(aviso)) return;
    if (!confirm('Confirma otra vez: se borrarán todas y no hay vuelta atrás.')) return;

    await Promise.all(objetivo.map(x => eliminarPresentacion(x.id)));
    setSelMes(null);
  };
  ...
  // Periodo en dos renglones: el taller arriba y el mes con el año debajo
  const lineasPeriodo = (p?: TipoPresentacion | null): { taller: string; fecha: string } => {
    if (!p) return { taller: '', fecha: '' };
    return {
      taller: (p.taller || '').trim(),
      fecha: [p.mes, p.ano].filter(Boolean).join(' · '),
    };
  };

  // Versión en una línea, para la barra superior y los rótulos
  ...
  const dibujarTexto = (texto: DiapositivaTexto, esPortada: boolean) => {
    ...
    return (
      <div style={{
        ...
        {/* El logo va suelto sobre el fondo, sin recuadro: solo una sombra
            suave para que despegue del color del taller. */}
        {taller?.logo && (
          <img
            src={taller.logo}
            alt={taller.nombre}
        ...
        {/* El título se parte en renglones por los separadores ":" y "-", y
            cada uno queda centrado bajo el logo, de mayor a menor. Así
            "KPI's : Key Performance Indicators - Indicadores clave de
            desempeño" se lee en tres líneas en lugar de un bloque corrido. */}
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', maxWidth: '1100px' }}>
          {partirTitulo(titulo).map((linea, i) => (
            <h1
              key={i}
              style={{
                margin: 0, fontWeight: 900, letterSpacing: '0.5px', lineHeight: 1.12,
                textShadow: '0 3px 12px rgba(0,0,0,0.35)', textAlign: 'center',
                fontSize: i === 0
                  ? 'clamp(2.4rem, 6.5vw, 4.4rem)'
                  : i === 1
                    ? 'clamp(1.5rem, 3.6vw, 2.5rem)'
        ...
        {(pie || texto.mostrarFecha || (texto.mostrarPeriodo && textoPeriodo(reproduciendo))) && (
          <div style={{ marginTop: '1rem', display: 'flex', flexDirection: 'column', gap: '0.35rem', opacity: 0.85 }}>
            {/* Taller arriba y, debajo, el mes con el año */}
            {texto.mostrarPeriodo && lineasPeriodo(reproduciendo).taller && (
              <span style={{ fontSize: 'clamp(1rem, 2.2vw, 1.35rem)', fontWeight: 800, letterSpacing: '0.5px' }}>
                {lineasPeriodo(reproduciendo).taller}
              </span>
            )}
            {texto.mostrarPeriodo && lineasPeriodo(reproduciendo).fecha && (
              <span style={{ fontSize: 'clamp(0.95rem, 2vw, 1.2rem)', fontWeight: 700, letterSpacing: '0.5px', opacity: 0.9 }}>
                {lineasPeriodo(reproduciendo).fecha}
              </span>
            )}
  ...
  // Bloque compacto de una diapositiva de texto (portada o conclusión).
  // Con la casilla apagada ocupa una sola línea; al encenderla muestra el
  // título y sus dos opciones en una fila, para que el formulario no crezca.
  const editorTexto = (
    titulo: string,
    ...
    // Casilla pequeña reutilizable
    const casilla = (activa: boolean, etiqueta: string, alPulsar: () => void, ayudita?: string) => (
      <div
        onClick={alPulsar}
        style={{ display: 'flex', alignItems: 'center', gap: '0.45rem', cursor: 'pointer', minWidth: 0 }}
      >
        <span style={{
          display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
          width: '17px', height: '17px', borderRadius: '5px', flexShrink: 0,
          backgroundColor: activa ? 'var(--primary)' : 'transparent',
          border: `2px solid ${activa ? 'var(--primary)' : 'var(--border)'}`, color: '#fff'
        }}>
          {activa && <Check size={11} />}
        </span>
        <span style={{ fontSize: '0.78rem', color: 'var(--text-main)', whiteSpace: 'nowrap' }}>
          {etiqueta}
          {ayudita && <small style={{ color: 'var(--text-muted)' }}> {ayudita}</small>}
        </span>
      </div>
    );
      ...
      <div style={{ border: '1px solid var(--border)', borderRadius: '10px', overflow: 'hidden' }}>
        <div
          ...
          style={{
            ...
            padding: '0.55rem 0.8rem', backgroundColor: 'var(--bg-highlight)'
          }}
        >
          <span style={{
            display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
            ...
            {valor.activa && <Check size={12} />}
          </span>
          <strong style={{ color: 'var(--text-main)', fontSize: '0.85rem' }}>{titulo}</strong>
          <small style={{ color: 'var(--text-muted)', fontSize: '0.72rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {ayuda}
          </small>
        </div>

        {valor.activa && (
          <div style={{ padding: '0.7rem 0.8rem', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
            <input
              className="form-control"
              style={{ width: '100%', boxSizing: 'border-box' }}
              value={valor.titulo}
              onChange={e => cambiar('titulo', e.target.value)}
              placeholder={titulo === 'Portada' ? "Título · ej: Presentación de KPI's" : 'Título · ej: Gracias'}
            />
            <div style={{ display: 'flex', alignItems: 'center', gap: '1.1rem', flexWrap: 'wrap' }}>
              {casilla(!!valor.mostrarPeriodo, 'Periodo', () => cambiar('mostrarPeriodo', !valor.mostrarPeriodo))}
              {casilla(!!valor.mostrarFecha, 'Fecha del día', () => cambiar('mostrarFecha', !valor.mostrarFecha))}
            </div>
          </div>
        )}
      </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
              ...
              {/* Edición masiva: cambia de un golpe todas las de la carpeta */}
              {puedoEditar && listaCarpeta.length > 1 && (
                <button
                  className="btn btn-outline"
                  onClick={abrirEditarTodas}
                  style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', color: 'var(--primary)', padding: '0.4rem 0.8rem', fontSize: '0.8rem' }}
                  title="Aplicar los mismos módulos, portada y conclusión a todas las presentaciones de esta carpeta"
                >
                  <Layers size={15} /> Editar todas ({listaCarpeta.length})
                </button>
              )}
              {/* Borrar el mes completo sin ir tarjeta por tarjeta */}
              {puedoEliminar && selMes && listaCarpeta.length > 0 && (
                <button
                  className="btn btn-outline"
                  onClick={eliminarCarpeta}
                  style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', color: 'var(--danger)', padding: '0.4rem 0.8rem', fontSize: '0.8rem' }}
                  title={`Eliminar las ${listaCarpeta.length} presentaciones de ${rutaCarpeta}`}
                >
                  <Trash2 size={15} /> Eliminar mes ({listaCarpeta.length})
                </button>
              )}
            </div>
          </div>
          ...
          {listaCarpeta.length === 0 ? (
            <div className="card" style={{ textAlign: 'center', padding: '4rem 2rem', margin: 0 }}>
              ...
              <p style={{ color: 'var(--text-muted)' }}>
                {puedoEditar ? 'Usa "Generar mes": se crea una presentación por cada taller, todas con los mismos módulos.' : 'Pide a un administrador que arme las presentaciones.'}
              </p>
            </div>
          ) : (
      ...
      {modalAbierto && (
        ...
        >
          <div
            ...
            style={{ backgroundColor: 'var(--bg-panel)', border: '1px solid var(--border)', borderRadius: '12px', width: '100%', maxWidth: '1280px', maxHeight: '92vh', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 50px rgba(0,0,0,0.5)' }}
          >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <div>
                  <h3 style={{ margin: 0, fontSize: '1.15rem', color: 'var(--text-main)' }}>
                    {modoEditor === 'una' ? 'Editar Presentación'
                      : modoEditor === 'masiva' ? 'Editar todas'
                      : 'Generar mes'}
                  </h3>
                  <p style={{ margin: '0.15rem 0 0 0', fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                    {modoEditor === 'una'
                      ? 'Los cambios afectan solo a la presentación de este taller'
                      : modoEditor === 'masiva'
                        ? `Los cambios se aplican a las ${listaCarpeta.length} presentaciones de ${rutaCarpeta}`
                        : `Se generará una presentación por cada taller (${talleresOrdenados.length}), todas con estos módulos`}
                  </p>
                </div>
              </div>
            ...
            <div style={{ padding: '1rem 1.5rem 1.25rem 1.5rem', overflowY: 'auto' }}>
              {/* TRES COLUMNAS: 1 periodo · 2 módulos · 3 orden */}
              <div className="pres-editor">
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
              {/* ---------- 1 · PERIODO ---------- */}
              <h4 className="pres-paso"><span className="pres-paso__num">1</span> Periodo y nombre</h4>
              <div style={{ border: '1px solid var(--border)', borderRadius: '10px', overflow: 'hidden' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', padding: '0.55rem 0.8rem', backgroundColor: 'var(--bg-highlight)' }}>
                  <Filter size={15} color="var(--primary)" style={{ flexShrink: 0 }} />
                  <small style={{ color: 'var(--text-muted)', fontSize: '0.72rem', lineHeight: 1.35 }}>
                    Define la carpeta, el nombre y el filtro con el que abren todas las diapositivas.
                  </small>
                </div>
                <div style={{ padding: '0.8rem', display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '0.75rem' }}>
                  ...
                  <div className="form-group" style={{ margin: 0, minWidth: 0 }}>
                    ...
                    {modoEditor === 'una' ? (
                      <select className="form-control" style={{ width: '100%', boxSizing: 'border-box' }} value={filtroTaller} onChange={e => setFiltroTaller(e.target.value)}>
                        ...
                        {talleresOrdenados.map(tl => <option key={tl.id} value={tl.nombre}>{tl.nombre}</option>)}
                      </select>
                    ) : (
                      <div
                        className="form-control"
                        style={{ width: '100%', boxSizing: 'border-box', backgroundColor: 'var(--bg-highlight)', color: 'var(--primary)', fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                        title={(modoEditor === 'masiva' ? listaCarpeta.map(x => x.taller || 'Sin taller') : talleresOrdenados.map(tl => tl.nombre)).join(', ')}
                      >
                        Todos ({modoEditor === 'masiva' ? listaCarpeta.length : talleresOrdenados.length})
                      </div>
                    )}
                  </div>
                  <div className="form-group" style={{ margin: 0, minWidth: 0 }}>
              ...
              {/* Nombre automático y descripción */}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '0.75rem' }}>
                <div className="form-group" style={{ margin: 0, minWidth: 0 }}>
                  <label className="form-label">Nombre <small style={{ color: 'var(--text-muted)', fontWeight: 400 }}>(automático)</small></label>
                  <div
                    className="form-control"
                    style={{
                      width: '100%', boxSizing: 'border-box', backgroundColor: 'var(--bg-highlight)',
                      color: nombre ? 'var(--primary)' : 'var(--text-muted)', fontWeight: 700,
                      overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap'
                    }}
                    title={nombre}
                  >
                    {nombre || 'Elige el año y el mes'}
                  </div>
                </div>
                <div className="form-group" style={{ margin: 0, minWidth: 0 }}>
              ...
              {/* ---------- 4 · APERTURA Y CIERRE ---------- */}
              <h4 className="pres-paso" style={{ marginTop: '0.35rem' }}><span className="pres-paso__num">4</span> Apertura y cierre</h4>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                {editorTexto('Portada', portada, setPortada, 'Escribe {mes}, {año} o {taller}')}
                {editorTexto('Conclusión', cierre, setCierre, 'Va después del último módulo')}
              </div>
              </div>

                {/* ---------- 2 · MÓDULOS ---------- */}
                <div>
                  <h4 className="pres-paso" style={{ marginTop: 0 }}><span className="pres-paso__num">2</span> Módulos a mostrar</h4>
                  <div style={{ border: '1px solid var(--border)', borderRadius: '10px', overflow: 'hidden', maxHeight: '52vh', overflowY: 'auto' }}>
                ...
                {/* ---------- 3 · ORDEN ---------- */}
                <div>
                  <h4 className="pres-paso" style={{ marginTop: 0 }}>
                    <span className="pres-paso__num">3</span> Orden de las diapositivas
                    {vistas.length > 0 && <span style={{ color: 'var(--primary)', marginLeft: '0.35rem' }}>({vistas.length})</span>}
                  </h4>
                  <div style={{ border: '1px solid var(--border)', borderRadius: '10px', overflow: 'hidden', maxHeight: '52vh', overflowY: 'auto' }}>
                    {vistas.length === 0 ? (
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem', padding: '1rem 1.5rem', borderTop: '1px solid var(--border)', flexShrink: 0 }}>
              ...
              <button className="btn btn-primary" onClick={guardar} disabled={guardando || !puedoEditar} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', opacity: (guardando || !puedoEditar) ? 0.55 : 1 }}>
                <Save size={16} /> {guardando ? 'Guardando...'
                  : modoEditor === 'una' ? 'Actualizar'
                  : modoEditor === 'masiva' ? `Aplicar a ${listaCarpeta.length}`
                  : 'Generar'}
              </button>
            </div>
          </div>
