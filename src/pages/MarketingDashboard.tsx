              ...
              <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', maxWidth: '1120px', minHeight: '104px', marginBottom: '0.5rem' }}>
                {tallerLogo && (
                  <img
                    ...
                    style={{ position: 'absolute', left: 0, top: '50%', transform: 'translateY(-50%)', width: '96px', height: '96px', objectFit: 'contain', filter: 'drop-shadow(0 4px 12px rgba(0,0,0,0.45))' }}
                  />
                )}
                <div style={{ padding: tallerLogo ? '0 112px' : 0, display: 'flex', justifyContent: 'center' }}>
                  <h3 style={{ margin: 0, fontSize: '1.4rem', fontWeight: 900, color: colorTextoSobre(tallerColor), letterSpacing: '0.5px', textAlign: 'center', lineHeight: 1.2, backgroundColor: tallerColor, padding: '0.55rem 1.6rem', borderRadius: '12px', boxShadow: '0 4px 14px rgba(0,0,0,0.35)', border: '2px solid rgba(255,255,255,0.25)' }}>
                    {tallerSeleccionado}
                    {/* El periodo va en su propio renglón, bajo el taller */}
                    <span style={{ display: 'block', fontSize: '1.05rem', fontWeight: 800, opacity: 0.88, marginTop: '0.15rem' }}>
                      {mes === 'Todos' ? ano : `${mes} · ${ano}`}
                    </span>
                  </h3>
                </div>
              </div>
